...
However, these proofs all work by showing that for any instance of an NP-hard problem, there exists a Bayesian network $P$ and an observation $y$ such that computing the posterior $P(X = \cdot | Y = y)$ would yield a solution to the NP-hard problem.
Just as exact inference is unnecessary in practice, the ability to do inference under any worst-case observed value $y$ is also unnecessary in many settings, and hence the worst-case nature of the value $y$ in these constructions is unsatisfying.
...
% For any Bayesian Bayesian network $(V, E, P)$ and any subset $Y \subseteq V$,
